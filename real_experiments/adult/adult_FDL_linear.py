...
def estimate_prob(A, y):
    classes = np.unique(y)
    combinations = list(product([0, 1], repeat=A.shape[1]))
    theta = {}
    ...
    for k in classes:
        ...
        for row in class_rows:
            counts[tuple(row)] += 1
        total = max(len(class_rows), 1)  # Avoid division by zero
        theta[k] = {combo: count/total for combo, count in counts.items()}
    return theta

def compute_prob(A, y, theta):
    return np.prod([theta[y[i]][tuple(row)] for i, row in enumerate(A)])

def sample_A(y, theta):
    combinations = list(theta[0].keys())
    return np.array([
        combinations[np.random.choice(len(combinations), p=list(theta[k].values()))]
        for k in y
    ])
...
def inner_func_test(n, dataset, dim, folder_name):
    ...
    theta = estimate_prob(A, Y)
    specified_A_tr = np.array([sample_A(Y, theta) for i in range(epochs_list[-1])])
        ...
        for lr_dis in lr_dis_list:
            for lr_loss in lr_loss_list:
                for mu_val in mu_val_list:
                    ...
                    model.fit(input_data_train, Y, check_period = epochs_list, specified_At = specified_A_tr)
...
if __name__ == '__main__':
    ...
    folder_name = f"./{dataset}_FDL_linear_{dim}dim_{mode}/"
